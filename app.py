...
def _set_meta(key, value):
    with engine.begin() as c:
        exists = c.execute(select(meta.c.key).where(meta.c.key == key)).fetchone()
        if exists:
            c.execute(update(meta).where(meta.c.key == key).values(value=str(value)))
        else:
            c.execute(insert(meta).values(key=key, value=str(value)))


def get_cash():
    return _get_meta("cash", 0.0)


def set_cash(amount):
    _set_meta("cash", amount)


def get_initial_capital():
    return _get_meta("initial_capital", 0.0)
